...
- **Despesas tab** (nav placed above Dashboard): Fretes-style table for daily per-fleet expenses (data, frota, cidade, motoristaNome/ajudanteNome (driver/assistant name text fields, searchable), frete, km, diesel, and 14 cost fields). Note: `motorista`/`ajudante` are numeric payment amounts; `motoristaNome`/`ajudanteNome` are the person names (text, not costs). `lucro` and `totalDespesa` are computed server-side (`lucro = frete − sum(14 cost fields)`; km/dieselLt are metrics, not costs). Same search+highlight, virtualization, context menus, CSV/Excel export as Fretes. Multiple records per same date are allowed (no unique constraint). Bulk import dedupes on all meaningful fields combined (data+frota+all 22 cost/text fields), skipping only exact duplicates.
